...
Por último evaluaremos la capacidad predictiva del modelo haciendo las 55 predicciones para el periodo correspondiente a prueba. Junio de 2019 hasta diciembre de 2023. Indicamos también el Lambda correspondiente para obtener las predicciones en la escala original.

```{r}
predictionHWAP_1=forecast::forecast(HWAP,h=55,level =0.95,lambda = -1.3)
predictionHWAP_1
...
La raíz del error cuadrático medio es 

```{r}
sqrt(mean((Test-predictionHWAP_1$mean)^2))
```
...
tsibbleBoxDesemTrain<-as_tsibble(TrainOriginal)#Corrigiendo un problema que tenía este objeto tsibble, por eso lo creé de nuevo.
Ajuste1=tsibbleBoxDesemTrain%>%
...
Pronostico1%>%autoplot(tsibbleBoxDesemTrain)+geom_line(aes(y=.fitted),col="red",data=augment(Ajuste1))+labs(y=" ",title="Pronóstico u ajustados")+guides(colour="none")
...
```{r}
sqrt(mean((Test-Pronostico1$.mean)^2))
```
