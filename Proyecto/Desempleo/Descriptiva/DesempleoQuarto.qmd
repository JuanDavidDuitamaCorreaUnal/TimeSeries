...
####Se crean las ventanas de rolling y se obUntiene los h-pronósticos para cada ventana(hay ntest posibles ventanas)
for(i in 1:(ntest)){
...
```{r}
modeloSARIMA = Arima(DesempleoTS, c(1, 1, 2),seasonal = list(order = c(2, 1, 1), period = 12),lambda = 1)
...
SARIMAtrain = Arima(train, c(1, 1, 2),seasonal = list(order = c(2, 1, 1), period = 12),lambda = 1)
resSARIMAtrain <- SARIMAtrain$residuals
coeftrain=coefs2poly(SARIMAtrain)
outliers= tsoutliers::locate.outliers(resSARIMAtrain,coeftrain)
outliers###tstat se compara con C=3
xregtrain = outliers.effects(outliers, ntrain)

SARIMAtrain_out = update(SARIMAtrain, xreg = xregtrain)
SARIMAtrain_out
```
...
h <- 1
n <- length(test) - h + 1
xregtest = cbind(matrix(0,n,2),matrix(1,n,4),matrix(0,n,1))
xregro <- rbind(xregtrain,xregtest)
fitmodelo <- update(SARIMAtrain_out,fixed=esta_param_modelo)
fc <- ts(numeric(n), start=c(2019,6), freq=12)
for(i in 1:n){ 
  x <- window(DesempleoTS, end=c(2019, 5+(i-1)))
  refit <- forecast::Arima(x, model=fitmodelo, xreg = xregro[1:(221+(i-1)),])
  fc[i] <- forecast::forecast(refit, h=h,xreg = t(as.matrix(xregro[(221+i),])))$mean[h]
}
dife=(test-fc)^2
...
## Rolling con datos recientes 

```{r}
newdata <- read_excel("C:/Users/EQUIPO/Documents/GitHub/TimeSeries/Proyecto/Desempleo/anex-GEIH-jul2024.xlsx", sheet = "Total nacional", range = "JR17:JX17",col_names = F)
...
n <- length(test) - h + 1
fitmodelo <- update(SARIMA_out,fixed=esta_param_modelo)
fc <- ts(numeric(n), start=c(2024,1), freq=12)
xregtemp = cbind(matrix(0,n,5),matrix(1,n,4),matrix(0,n,2))
xregnew <- rbind(xreg,xregtemp)

DesempTSnew <- ts(c(DesempleoTS, newdataTS), start = start(DesempleoTS), frequency = 12)

for(i in 1:n){ 
  x <- window(DesempTSnew, end=c(2023, 12+(i-1)))
  refit <- forecast::Arima(x, model=fitmodelo, xreg = xregnew[1:(276+(i-1)),])
  fc[i] <- forecast::forecast(refit, h=h,xreg = t(as.matrix(xregnew[277,])))$mean[h]
}
dife=(test-fc)^2
ecm=(1/(length(test)))*sum(dife)
ecm
...
## SARIMA que pasó los supuestos vs Datos reales
