...
setwd("C:/Users/EQUIPO/OneDrive/Documentos/GitHub/TimeSeries/Proyecto/Desempleo")
...
\end{align*}
```
El siguiente ejemplo realiza un primer suavizamiento exponencial únicamente con la componente de Nivel sobre la tasa de desempleo con los datos originales; es decir, el único parámetro que se utilizará es $\alpha$.
...
La estimación del parámetro de Nivel alpha es 0.3114955, de Pendiente beta 0.02446716 y de estacionalidad gamma 0.4501506.

Por último evaluaremos la capacidad predictiva del modelo haciendo las 55 predicciones para el periodo correspondiente a prueba. Junio de 2019 hasta diciembre de 2023. Indicamos también el Lambda 1 ya que se trabajo con la serie sin transformar.
...
Vemos que el error cuadrático medio de las predicciones un paso adelante tanto en el rolling manual como utilizando la función son prácticamente iguales.

#### Aquí acaba el rolling :b

Usando objetos tsibble ahora

```{r}
TrainOriginal=ts(DesempleoTS[1:221], start = c(2001, 1),frequency = 12)
tsibbleDesemTrain<-as_tsibble(TrainOriginal)#Corrigiendo un problema que tenía este objeto tsibble, por eso lo creé de nuevo.
Ajuste1=tsibbleDesemTrain%>%
  model(ETS(value~ error("A")+trend("A")+season("A")))

Pronostico1=Ajuste1%>%
  fabletools::forecast(h=55)
Pronostico1

Pronostico1%>%autoplot(tsibbleDesemTrain)+geom_line(aes(y=.fitted),col="red",data=augment(Ajuste1))+labs(y=" ",title="Pronóstico u ajustados")+guides(colour="none")
...
El procedimiento inicia con la creación de un objeto tibble de la serie.

```{r}
TibDesem<-as_tibble(tsibbleDesem)
TibDesem$index<-as.Date(TibDesem$index)
TibDesem<-rename(TibDesem,Fecha=index,Desem=value)
TibDesem<-TibDesem%>%mutate(Original=box_cox_inv_vec(Desem,lambda = Lambda))
```

Posteriormente se divide la serie en conjunto de entrenamiento y de prueba.

```{r}
Particion=timetk::time_series_split(TibDesem,date_var=Fecha,assess = 12,cumulative = TRUE)#En este caso, al decir assess=12 indica que las últimas 12 observaciones serán utilizadas como conjunto de prueba
Particion%>% tk_time_series_cv_plan()%>%
  plot_time_series_cv_plan(Fecha,Desem)
...
ModeloEWMA<-modeltime::exp_smoothing(
  ...
  set_engine("ets")%>%
  fit(Desem ~ Fecha,data=training(Particion))
