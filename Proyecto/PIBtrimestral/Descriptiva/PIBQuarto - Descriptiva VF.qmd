...
co=0.14422####Valor del cuantil aproximado para cusumsq para n/2 (valor común usado)
...
#### Rolling con datos ARIMA (0,1,0)

```{r}
h=1
lserie=length(PIB3TS)
ntrain <- length(ts_train_PIB)
ntrain
time(PIB3TS)
...
ntest=length(ts_test_PIB)
ntest
fcmat=matrix(0,nrow=ntest,ncol=h)
for(i in 1:ntest){
  x=window(PIB3TS,end=c(2020,1+(i-1)))
  refit=forecast::Arima(x,order = c(0,1,0),lambda = LambdaPIB,include.constant = TRUE)
...
```{r}
pronosticos_PIB=forecast::forecast(modelo_ajustado,h=12,level=0.95)
plot(pronosticos_PIB)
```
